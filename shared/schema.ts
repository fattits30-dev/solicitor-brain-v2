...
// Cases table (fixed to match actual database structure)
export const cases = pgTable('cases', {
  id: serial('id').primaryKey(),
  caseReference: varchar('case_reference', { length: 100 }).notNull().unique(),
...
export const persons = pgTable('persons', {
  ...
  personType: varchar('person_type', { length: 50 }).notNull(),
...
export const documents = pgTable(
  ...
  (table) => ({
    caseIdIndex: index('idx_documents_case_id').on(table.caseId),
    hashIndex: index('idx_documents_hash').on(table.hash),
  }),
);
...
export const events = pgTable(
  ...
  (table) => ({
    caseIdIndex: index('idx_events_case_id').on(table.caseId),
    dateIndex: index('idx_events_date').on(table.eventDate),
  }),
);
...
export const drafts = pgTable('drafts', {
  id: serial('id').primaryKey(),
  caseId: integer('case_id').references(() => cases.id, { onDelete: 'cascade' }),
  draftType: varchar('draft_type', { length: 100 }).notNull(),
  title: varchar('title', { length: 500 }).notNull(),
...
export const embeddings = pgTable(
  ...
  (table) => ({
    documentIdIndex: index('idx_embeddings_document_id').on(table.documentId),
    documentChunkIndex: index('embeddings_document_id_chunk_index_key').on(table.documentId, table.chunkIndex),
  }),
);
