...
export default function Login() {
  ...
  const { login, mfaRequired, mfaStatus, completeMfaVerification } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        if (response.mfaRequired && response.mfaStatus) {
          // MFA is required - the context will handle setting mfaRequired state
  ...
  const handleMfaSuccess = () => {
    completeMfaVerification({ deviceTrusted: false });
  ...
  const handleMfaFailure = (errorMessage: string) => {
    setError(`MFA verification failed: ${errorMessage}`);
  };

  const handleCloseMfa = () => {
    // Reset only the error, let user try MFA again
    setError('');
  };
    ...
    <>
      ...
      {mfaRequired && mfaStatus && (
        <MfaVerificationModal
          ...
          onVerificationFailure={handleMfaFailure}
        />
      )}
