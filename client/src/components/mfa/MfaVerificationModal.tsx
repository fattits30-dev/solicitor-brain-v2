...
export function MfaVerificationModal({
  ...
  const handleVerification = async (method: VerificationMethod, code: string) => {
    try {
      ...
      const response = await fetch(`/api/mfa/verify/${method}`, {
        method: 'POST',
        headers: {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[500px]" onPointerDownOutside={(e) => e.preventDefault()}>
        ...
        <div className="space-y-4">
          ...
          <Tabs value={activeMethod} onValueChange={(value) => setActiveMethod(value as VerificationMethod)}>
              ...
              {!emailSent ? (
                ...
                <div className="space-y-4">
                  <Alert>
                    <Mail className="h-4 w-4" />
                    <AlertDescription>
                      Verification code sent to your registered email address. Check your inbox and spam folder.
                    </AlertDescription>
                  </Alert>
