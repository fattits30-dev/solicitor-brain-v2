...
interface _DocumentViewerProps {
  documentId?: string;
  documentUrl?: string;
  documentName?: string;
  documentType?: string;
...
interface _DocumentUploadProps {
  caseId?: string;
...
interface _DocumentMetadataProps {
  documentId: string;
...
export const detectDocumentType = (filename: string): string => {
  ...
  if (name.includes('letter') || name.includes('email') || name.includes('correspondence'))
    return DOCUMENT_TYPES.CORRESPONDENCE;
  ...
  if (name.includes('court') || name.includes('filing') || name.includes('motion'))
    return DOCUMENT_TYPES.COURT_FILING;
