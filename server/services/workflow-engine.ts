...
export interface WorkflowTemplate {
  ...
  name: string;
  description: string;
  category: 'litigation' | 'conveyancing' | 'probate' | 'employment' | 'family' | 'commercial' | 'personal_injury';
...
export interface WorkflowTask {
  ...
  name: string;
  description: string;
  type: 'document_draft' | 'form_completion' | 'research' | 'client_contact' | 'court_filing' | 'negotiation' | 'review' | 'other';
...
class WorkflowEngineService {
          ...
          tasks: [
            {
              ...
              checklist: [
                { id: 'contracts_reviewed', description: 'Original contracts reviewed', required: true },
                { id: 'invoices_validated', description: 'Invoices and statements validated', required: true },
            ...
            },
            {
              ...
              checklist: [
                { id: 'company_search', description: 'Companies House search completed', required: true },
                { id: 'land_registry', description: 'Land Registry search if applicable', required: false },
            ...
            },
            {
              ...
              checklist: [
                { id: 'recovery_prospects', description: 'Recovery prospects assessed', required: true },
                { id: 'cost_benefit', description: 'Cost-benefit analysis completed', required: true }
              ]
            }
          ],
          milestones: [
          ...
          tasks: [
            {
              ...
              checklist: [
                { id: 'letter_drafted', description: 'Letter drafted with all required elements', required: true },
                { id: 'letter_sent', description: 'Letter sent by recorded delivery', required: true },
                { id: 'response_deadline', description: 'Response deadline clearly stated', required: true }
              ]
            },
            {
              ...
              checklist: [
                { id: 'response_received', description: 'Any response received recorded', required: true },
                { id: 'response_analyzed', description: 'Response content analyzed', required: false }
              ]
            },
            {
              ...
              automation_available: false,
              checklist: [
                { id: 'settlement_attempted', description: 'Settlement discussions attempted', required: true },
                { id: 'terms_documented', description: 'Any agreed terms documented', required: false }
              ]
            }
          ],
          milestones: [
          ...
          tasks: [
            {
              ...
              checklist: [
                { id: 'claim_form_complete', description: 'Claim form completed accurately', required: true },
                { id: 'particulars_attached', description: 'Particulars of claim attached', required: true },
                { id: 'court_fee_calculated', description: 'Correct court fee calculated', required: true }
              ]
            },
            {
              id: 'court_issue',
              ...
              checklist: [
                ...
                { id: 'case_number_received', description: 'Case number allocated', required: true }
              ]
            },
            {
              ...
              automation_available: false,
              checklist: [
                { id: 'service_method_chosen', description: 'Appropriate service method chosen', required: true },
                { id: 'service_effected', description: 'Service effected properly', required: true },
                { id: 'certificate_filed', description: 'Certificate of service filed', required: true }
              ]
            }
          ],
          milestones: [
      ...
      stages: [
        {
          ...
          tasks: [
            ...
            },
            {
              ...
              automation_available: false,
              checklist: [
                { id: 'local_search', description: 'Local authority search ordered', required: true },
                { id: 'water_search', description: 'Water authority search ordered', required: true },
                { id: 'environmental_search', description: 'Environmental search ordered', required: false }
              ]
            }
          ],
          milestones: [
  ...
  async createCaseWorkflow(
    ...
    const totalDays = template.stages.reduce((sum, stage) => sum + stage.estimated_duration_days, 0);
   ...
   */
  getNextTasks(workflow: CaseWorkflow, assigneeRole?: string): CaseWorkflowTask[] {
    ...
    const nextTasks = currentStage.tasks.filter(task => {
      ...
      if (assigneeRole && taskTemplate?.assignee_role !== assigneeRole) {
        return false;
      }
  ...
  async completeTask(
    ...
    console.log(`Completing task ${taskId} in workflow ${workflowId}`);
  ...
  async autoAssignTasks(
    ...
    try {
      ...
      const assignmentPrompt = `Optimize task assignments for workflow ${workflowId}:
      ...
      const assignments = await aiService.generateDraft(
        assignmentPrompt,
   ...
   */
  async generateProgressReport(workflowId: string): Promise<{
    summary: string;
  ...
  async triggerTaskAutomation(
    ...
    try {
      ...
      const template = this.getWorkflowTemplate(workflowId);
   ...
   */
  private async triggerDocumentAutomation(taskId: string, caseData: any, clientData: any): Promise<any> {
  ...
  private async triggerFormAutomation(taskId: string, caseData: any, clientData: any): Promise<any> {
  ...
  private async triggerDeadlineAutomation(taskId: string, caseData: any): Promise<any> {
    try {
   ...
   */
  async generateWorkflowOptimizations(workflowId: string): Promise<{
    ...
    try {
      const optimization = await aiService.generateDraft(
        `Analyze workflow ${workflowId} and suggest optimizations for:
   ...
   */
  getWorkflowMetrics(templateId: string): {
    ...
    return {
      ...
      success_rate: 0.92,
      common_bottlenecks: [
        { stage: 'court_proceedings', average_delay: 7 }
      ],
      automation_usage: [
        { task: 'letter_before_action', automation_rate: 0.85 }
      ],
