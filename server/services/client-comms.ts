...
export interface CommunicationTemplate {
  id: string;
  name: string;
  category: 'client_care' | 'progress_update' | 'billing' | 'appointment' | 'completion' | 'compliance' | 'marketing';
...
export interface AutoSendTrigger {
  event: 'case_opened' | 'milestone_reached' | 'deadline_approaching' | 'payment_due' | 'document_ready' | 'appointment_scheduled';
  delay_hours?: number;
  conditions?: Array<{
    field: string;
...
export interface AppointmentCommunication {
  ...
  client_id: string;
  appointment_type: 'initial_consultation' | 'progress_meeting' | 'court_hearing' | 'signing' | 'other';
  date_time: Date;
  location: string;
...
This communication is confidential and may be legally privileged. If you are not the intended recipient, please notify us immediately.`,
      ...
      variables: [
        { name: 'client_name', description: 'Client full name', type: 'text', required: true, source: 'client.name' },
        { name: 'case_title', description: 'Short case title', type: 'text', required: true, source: 'case.title' },
        { name: 'case_description', description: 'Brief case description', type: 'text', required: true, source: 'case.description' },
        { name: 'matter_reference', description: 'Matter reference number', type: 'text', required: true, source: 'case.reference' },
        { name: 'solicitor_name', description: 'Solicitor name', type: 'text', required: true, source: 'solicitor.name' },
        { name: 'estimated_costs', description: 'Estimated costs', type: 'currency', required: true }
      ],
...
{{firm_name}}`,
      ...
      variables: [
        { name: 'current_status', description: 'Current case status', type: 'text', required: true },
        { name: 'recent_developments', description: 'Recent developments', type: 'text', required: true },
        { name: 'next_steps', description: 'Next steps planned', type: 'text', required: false },
        { name: 'costs_to_date', description: 'Costs incurred to date', type: 'currency', required: true }
      ],
...
{{firm_name}}`,
      ...
      variables: [
        ...
        { name: 'appointment_location', description: 'Meeting location', type: 'address', required: true },
        { name: 'appointment_purpose', description: 'Purpose of meeting', type: 'text', required: true }
      ],
...
{{firm_name}}`,
      ...
      variables: [
        ...
        { name: 'work_summary', description: 'Summary of work done', type: 'text', required: false }
      ],
...
{{firm_name}}`,
      ...
      auto_send_triggers: [
        {
          event: 'milestone_reached',
          conditions: [
            { field: 'case_status', operator: 'equals', value: 'completed' }
          ]
        }
      ],
      variables: [
        { name: 'outcome_summary', description: 'Summary of case outcome', type: 'text', required: true },
        { name: 'final_costs', description: 'Total final costs', type: 'currency', required: true },
        { name: 'retention_period', description: 'File retention period', type: 'number', required: true, default_value: '7' }
      ],
  ...
  async generateCommunication(
    ...
    const enrichedVariables = await this.autoPopulateVariables(template, variables, caseData, clientData);
  ...
  async sendClientUpdate(update: ClientUpdate): Promise<CommunicationRecord> {
    try {
      ...
      const communication = await this.generateCommunication(
        'progress-update',
        {
   ...
   */
  async scheduleAppointmentCommunications(appointment: AppointmentCommunication): Promise<Array<{
  ...
  async generateInvoiceCommunication(billing: BillingCommunication): Promise<CommunicationRecord> {
    const workSummary = billing.time_entries
      .map(entry => `${format(entry.date, 'dd/MM/yyyy')}: ${entry.description} (${entry.hours}h @ £${entry.rate}/h = £${entry.amount})`)
      .join('\n');
  ...
  private async autoPopulateVariables(
    ...
    for (const variable of template.variables) {
      ...
      if (variable.source) {
        ...
        const sourceData = source === 'client' ? clientData : 
                          source === 'case' ? caseData :
                          source === 'solicitor' ? { name: 'Solicitor Name', email: 'solicitor@firm.com' } : null;
   ...
   */
  getCommunicationMetrics(templateId?: string): {
    total_sent: number;
  ...
  async generateMassCommunication(
    ...
    return {
      communications,
      errors,
      scheduled_send_time: addDays(new Date(), send_delay_hours / 24)
    };
  }
}
