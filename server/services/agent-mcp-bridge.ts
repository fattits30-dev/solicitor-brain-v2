...
export class AgentMCPBridge {
  ...
  /**
   * Execute filesystem operations for agents using MCP filesystem service
   */
  async fileOperation(operation: string, params: any): Promise<any> {
    try {
      switch (operation) {
        case 'read':
          return await mcpFilesystem.readTextFile(params.path, params.options);
        
        case 'write':
          await mcpFilesystem.writeFile(params.path, params.content);
          return { success: true, path: params.path, method: mcpFilesystem.isMCPEnabled() ? 'mcp' : 'fallback' };
        
        case 'list': {
          const files = await mcpFilesystem.listDirectory(params.path, params.includeSize);
          return files.map(f => ({ name: f.name, type: f.type, size: f.size }));
        }
        
        case 'exists':
          return await mcpFilesystem.exists(params.path);
        
        case 'info':
          return await mcpFilesystem.getFileInfo(params.path);
        
        case 'search':
          return await mcpFilesystem.searchFiles(
        ...
        case 'hash':
          return await mcpFilesystem.calculateFileHash(params.path);
        
        case 'createDir':
          await mcpFilesystem.createDirectory(params.path);
          return { success: true, path: params.path, method: mcpFilesystem.isMCPEnabled() ? 'mcp' : 'fallback' };
        
        case 'delete':
          await mcpFilesystem.deleteFile(params.path);
          return { success: true, path: params.path, method: 'native' }; // Delete always uses native fs
        
        case 'allowedDirs':
          return await mcpFilesystem.getAllowedDirectories();
      ...
      if (mcpFilesystem.isMCPEnabled()) {
        console.warn(`MCP filesystem operation failed, attempting fallback for ${operation}:`, error);
        ...
        switch (operation) {
          ...
          case 'write':
            ...
            return { success: true, path: params.path, method: 'fallback' };
          
          case 'list': {
            const files = await fs.readdir(params.path);
            return files.map(name => ({ name, type: 'unknown' }));
          }
...
1. FILESYSTEM:
   ...
   [FILE:SEARCH:basePath:pattern] - Search for files
   [FILE:HASH:path] - Calculate file hash
   [FILE:CREATEDIR:path] - Create directory
   [FILE:DELETE:path] - Delete file
   [FILE:ALLOWEDDIRS] - Get allowed directories
        ...
        case 'FILE':
          ...
          if (params[1]) {
            if (operation.toLowerCase() === 'write') {
              request.params.content = params[1];
            } else if (operation.toLowerCase() === 'search') {
              request.params.basePath = params[0];
              request.params.pattern = params[1];
              if (params[2]) request.params.excludePatterns = params[2].split(',');
            }
          }
          break;
