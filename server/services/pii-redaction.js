...
class PIIRedactionService {
  ...
  redact(text, options = {}) {
    ...
    const report = generateReport
      ? this.generateRedactionReport(text, redactedText, redactionLog, statistics)
      : null;
  ...
  getPatternsForLevel(level, excludePatterns, includePatterns) {
    ...
    switch (level) {
      ...
      case 'standard':
        ...
        patterns = Object.keys(this.patterns).filter((p) => !this.patterns[p].excludeFromDefault);
        break;
  ...
  generateRedactionReport(originalText, redactedText, log, statistics) {
    return {
      summary: {
        ...
        redactionPercentage:
          ((statistics.redactedItems / originalText.split(/\s+/).length) * 100).toFixed(2) + '%',
      },
      byCategory: {
        personalIdentifiers: log.filter((l) => ['nino', 'passport', 'nhsNumber'].includes(l.type))
          .length,
        financialData: log.filter((l) =>
          ['creditCard', 'ukBankAccount', 'sortCode'].includes(l.type),
        ).length,
  ...
  generateRecommendations(log, statistics) {
    ...
    if (statistics.redactedItems === 0) {
      recommendations.push(
        'No PII detected. Consider reviewing if document should contain personal information.',
      );
    }

    if (statistics.redactedItems > 50) {
      recommendations.push(
        'High volume of PII detected. Consider if this document should be handled with extra security measures.',
      );
    }
    ...
    if (hasNINO) {
      recommendations.push(
        'National Insurance Numbers detected. Special handling required under UK data protection laws.',
      );
    }
