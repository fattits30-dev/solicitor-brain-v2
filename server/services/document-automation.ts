...
{{party1Name}}                         {{party2Name}}`,
  ...
  async generateParticularsOfClaim(data: ParticularsOfClaimData): Promise<GeneratedDocument> {
    try {
      ...
      const _structuredFacts = await aiService.generateDraft(
      ...
      console.error('AI assistance failed for particulars');
      // Fallback without AI
  ...
  async generateWitnessStatement(
    ...
    try {
      ...
      console.error('AI assistance failed for witness statement');
      // Fallback without AI
      const data = {
        witnessName,
        witnessAddress,
  ...
  async generateLetterBeforeAction(
    ...
    try {
      ...
      console.error('AI assistance failed for letter before action');
      // Fallback without AI
      const data = {
        clientName,
        defendantName,
  ...
  private checkCompliance(
    ...
    if (template.category === 'pleading') {
      if (
        !content.includes('BETWEEN:') &&
        !content.includes('Claimant') &&
  ...
  async reviewDocument(
    ...
    try {
      ...
      // Parse AI review and extract actionable feedback
      const lines = review.split('\n').filter((line) => line.trim());
      const suggestions: Array<{ type: 'error' | 'warning' | 'suggestion'; message: string }> = [];
      const complianceIssues: string[] = [];
      const improvements: string[] = [];

      // Extract suggestions from AI response
      for (const line of lines) {
        if (line.toLowerCase().includes('error') || line.toLowerCase().includes('missing')) {
          suggestions.push({ type: 'error', message: line.trim() });
        } else if (
          line.toLowerCase().includes('warning') ||
          line.toLowerCase().includes('should')
        ) {
          suggestions.push({ type: 'warning', message: line.trim() });
        } else if (
          line.toLowerCase().includes('suggest') ||
          line.toLowerCase().includes('consider')
        ) {
          suggestions.push({ type: 'suggestion', message: line.trim() });
          improvements.push(line.trim());
        }

        if (line.toLowerCase().includes('compliance') || line.toLowerCase().includes('cpr')) {
          complianceIssues.push(line.trim());
        }
      }

      // Calculate score based on content quality
      let overallScore = 8.0;
      if (content.length < 200) overallScore -= 1.0;
      if (!content.includes('Statement of Truth')) overallScore -= 1.0;
      if (suggestions.filter((s) => s.type === 'error').length > 0) overallScore -= 2.0;

      return {
        overallScore: Math.max(5.0, overallScore),
        suggestions:
          suggestions.length > 0
            ? suggestions
            : [{ type: 'suggestion', message: 'Document structure appears acceptable' }],
        complianceIssues,
        improvements,
      };
    } catch {
      console.error('AI document review failed');
      return {
        overallScore: 7.0,
        suggestions: [
          { type: 'warning', message: 'Automated review unavailable - please manually review' },
        ],
        complianceIssues: [],
        improvements: [],
      };
