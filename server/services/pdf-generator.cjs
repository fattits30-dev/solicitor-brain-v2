...
class PDFGenerator {
  ...
  async generateDocument(options) {
    const { title, content, _type, _metadata = {} } = options;
    ...
    return new Promise((resolve, reject) => {
      ...
      // Add header
      this.addHeader(doc, _type);

      // Add title
      doc.fontSize(16).font(this.boldFont).text(title, { align: 'center' }).moveDown();

      // Add metadata if present
      if (_metadata.caseNumber) {
        doc
          .fontSize(11)
          .font(this.defaultFont)
          .text(`Case Number: ${_metadata.caseNumber}`, { align: 'center' })
          .moveDown();
      }

      if (_metadata.date) {
        doc.text(`Date: ${_metadata.date}`, { align: 'center' }).moveDown(2);
      }

      // Add content
      this.addFormattedContent(doc, content);

      // Add footer
      this.addFooter(doc, _metadata);
   ...
   */
  addFormattedContent(doc, content) {
    ...
    for (const line of lines) {
      ...
      if (/^[A-Z\s]+:/.test(line) || /^\d+\./.test(line)) {
        ...
        doc.font(this.boldFont).fontSize(12).text(line, { align: 'left' });
        inSection = true;
      }
      // Handle subsection headers
      else if (/^\w+:/.test(line) && line.length < 50) {
        doc.font(this.boldFont).fontSize(11).text(line, { align: 'left' });
      }
      // Handle numbered lists
      else if (/^\d+\.\s/.test(line)) {
        doc.font(this.defaultFont).fontSize(11).text(line, { indent: 20 });
      }
      // Handle bullet points
      else if (/^[•-]\s/.test(line)) {
        doc.font(this.defaultFont).fontSize(11).text(line, { indent: 30 });
      }
      // Regular text
      else if (line.trim()) {
        doc.font(this.defaultFont).fontSize(11).text(line, { align: 'justify' });
      }
      // Empty line
      else {
        doc.moveDown(0.5);
  ...
  addHeader(doc, type) {
    ...
    doc.fontSize(10).font(this.defaultFont).text(headerText, { align: 'center' }).moveDown();
  ...
  addFooter(doc, metadata) {
    ...
    doc
      .fontSize(9)
      .font(this.defaultFont)
      .text(`Page ${pageNumber}`, 0, doc.page.height - 50, {
        align: 'center',
    ...
    if (metadata.confidential) {
      doc
        .fontSize(8)
        .text('CONFIDENTIAL - LEGALLY PRIVILEGED', 0, doc.page.height - 35, {
          align: 'center',
          width: doc.page.width,
        });
    }
  ...
  addCoverPage(doc, caseData) {
    doc.fontSize(20).font(this.boldFont).text('CASE BUNDLE', { align: 'center' }).moveDown(2);

    doc.fontSize(16).text(caseData.title, { align: 'center' }).moveDown();
  ...
  addIndex(doc, documents) {
    doc.fontSize(14).font(this.boldFont).text('INDEX', { align: 'center' }).moveDown(2);

    doc.fontSize(11).font(this.defaultFont);

    documents.forEach((document, index) => {
      const pageNum = index + 3; // After cover and index
      doc
        .text(`${index + 1}. ${document.title}`, { continued: true })
  ...
  addBundleDocument(doc, document, number) {
    ...
    doc.fontSize(11).font(this.defaultFont).text(document.content, { align: 'justify' });
  }
}
