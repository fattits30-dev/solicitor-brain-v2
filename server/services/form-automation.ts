  ...
  private async extractDataFromDocument(
    ...
    try {
      const fieldDescriptions = template.fields
        .map((f) => `${f.name}: ${f.label} (${f.type}) - ${f.help_text || ''}`)
        .join('\n');
...
Use null for fields where no suitable data is found.`;
  ...
  validateFormSubmission(
    ...
    for (const field of template.fields) {
      ...
      if (field.max_length && value && value.toString().length > field.max_length) {
        errors.push({
          field: field.name,
          message: `${field.label} is too long (max ${field.max_length} characters)`,
        });
      }
    }
  ...
  async submitForm(
    ...
    if (!validation.is_valid) {
      throw new Error(
        `Form validation failed: ${validation.errors.map((e) => e.message).join(', ')}`,
      );
    }
   ...
   */
  getFormStatistics(_formId: string): {
