...
class DeadlineCalculatorService {
  ...
  private calculateBusinessDay(startDate: Date, daysToAdd: number): Date {
    ...
    while (daysAdded < daysToAdd) {
      ...
      if (!isWeekend(currentDate)) {
        ...
        const isPublicHoliday = this.publicHolidays2024.some(
          (holiday) => format(holiday, 'yyyy-MM-dd') === format(currentDate, 'yyyy-MM-dd'),
        );
  ...
  getApproachingDeadlines(deadlines: Deadline[], daysAhead = 7): Deadline[] {
    ...
    return deadlines
      .filter((deadline) => deadline.status === 'pending' && deadline.dueDate <= cutoffDate)
  ...
  async analyzeCaseForDeadlines(
    ...
    try {
      ...
      try {
        ...
        for (const dateInfo of identifiedDates) {
          if (dateInfo.confidence > 0.7) {
            ...
            if (
              dateInfo.event.toLowerCase().includes('cause of action') ||
              dateInfo.event.toLowerCase().includes('incident')
            ) {
              const limitationDeadlines = await this.generateCaseDeadlines(
                caseId,
                'personal_injury',
                {
      ...
      return {
        ...
        suggestedDeadlines: [],
        riskAssessment:
          'Unable to perform automated deadline analysis. Please manually review case documents for key dates.',
      };
    }
  }
