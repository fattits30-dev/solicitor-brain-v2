...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // AI Features routes (protected) - Comprehensive AI assistant
  ...
  app.get('/api/cases', authenticate, async (req, res) => {
    try {
      console.log('Cases route: Starting...');
      const cases = await storage.getCases();
      console.log('Cases route: Got results:', cases?.length, 'cases');
      res.json(cases);
    } catch (error) {
      console.error('Cases route: Error occurred:', error);
  ...
  app.post('/api/cases', authenticate, async (req, res) => {
    try {
      ...
      logDataModification(req, 'CREATE', 'cases', newCase.id.toString(), newCase);
  ...
  app.post('/api/cases/:caseId/drafts', async (req, res) => {
    try {
      ...
      await storage.createAuditEntry({
        ...
        resource: 'draft',
        resourceId: newDraft.id.toString(),
      });
  ...
  app.patch('/api/documents/:id/metadata', authenticate, async (req, res) => {
    try {
      ...
      await storage.createAuditEntry({
        userId: req.user?.id?.toString() || 'system',
  ...
  // Debug WebSocket services removed in production cleanup
