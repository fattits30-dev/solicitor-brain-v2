...
const TOKEN = 'test-token-123'; // Would normally get from login
...
async function uploadCases() {
  ...
  for (const caseInfo of cases) {
    ...
    // Create the case
    try {
      const caseResponse = await axios.post(`${API_URL}/cases`, {
      ...
      if (fs.existsSync(documentsPath)) {
        ...
        for (const doc of documents) {
          ...
          try {
            ...
            console.error(`    ✗ Failed to upload ${doc}:`, error.response?.data || error.message);
          }
        }
      }

      // Upload case summary JSON
      ...
      if (fs.existsSync(summaryPath)) {
        ...
        try {
          ...
          console.error(`    ✗ Failed to upload summary:`, error.response?.data || error.message);
        }
      }
      ...
      console.error(`  ✗ Failed to create case ${caseInfo.name}:`, error.response?.data || error.message);
    }
  }
